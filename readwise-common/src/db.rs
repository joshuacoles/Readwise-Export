use crate::{Library, Tag, ReadwiseObjectKind};
use anyhow::Context;
...
impl Database {
    ...
    pub async fn insert_books(&self, books: &[&crate::library::Book]) -> anyhow::Result<()> {
        if books.is_empty() {
            return Ok(());
        }
        ...
        for book in books {
            ...
            book_last_highlight_ats.push(book.last_highlight_at.as_ref().map(|dt| dt.to_rfc3339()));
            book_updateds.push(book.updated.as_ref().map(|dt| dt.to_rfc3339()));
        ...
        for i in 0..books.len() {
            query = query
                ...
                .bind(book_last_highlight_ats[i].clone())
                .bind(book_updateds[i].clone())
    ...
    pub async fn insert_highlight(&self, highlight: &crate::library::Highlight) -> anyhow::Result<()> {
        self.insert_highlights(&[highlight]).await
    }

    pub async fn insert_highlights(&self, highlights: &[&crate::library::Highlight]) -> anyhow::Result<()> {
        ...
        for highlight in highlights {
            ...
            highlight_highlighted_ats.push(highlight.highlighted_at.as_ref().map(|dt| dt.to_rfc3339()));
            highlight_urls.push(highlight.url.as_deref());
            highlight_colors.push(&highlight.color);
            highlight_updateds.push(highlight.updated.to_rfc3339());
        ...
        for i in 0..highlights.len() {
            query = query
                ...
                .bind(highlight_highlighted_ats[i].clone())
                .bind(highlight_urls[i])
                .bind(highlight_colors[i])
                .bind(highlight_updateds[i].clone())
    ...
    pub async fn insert_document(&self, document: &crate::library::Document) -> anyhow::Result<()> {
        self.insert_documents(&[document]).await
    }

    pub async fn insert_documents(&self, documents: &[&crate::library::Document]) -> anyhow::Result<()> {
        ...
        for document in documents {
            let published_date = match &document.published_date {
                Some(published_date) => Some(published_date.to_rfc3339()),
                None => None,
            };
            ...
            document_created_ats.push(document.created_at.to_rfc3339());
            document_updated_ats.push(document.updated_at.to_rfc3339());
            ...
            document_first_opened_ats.push(document.first_opened_at.as_ref().map(|dt| dt.to_rfc3339()));
            document_last_opened_ats.push(document.last_opened_at.as_ref().map(|dt| dt.to_rfc3339()));
            document_saved_ats.push(document.saved_at.to_rfc3339());
            document_last_moved_ats.push(document.last_moved_at.to_rfc3339());
        }
        ...
        for i in 0..documents.len() {
            query = query
                ...
                .bind(document_created_ats[i].clone())
                .bind(document_updated_ats[i].clone())
                ...
                .bind(document_first_opened_ats[i].clone())
                .bind(document_last_opened_ats[i].clone())
                .bind(document_saved_ats[i].clone())
                .bind(document_last_moved_ats[i].clone());
        }
    ...
    pub async fn insert_tags(&self, tags: &[&crate::Tag]) -> anyhow::Result<()> {
        if tags.is_empty() {
            return Ok(());
        }
